...
mod tests {
    ...
    ///
    /// This test uses the static assertions generated by wgsl_bindgen to ensure
    /// that the memory layout of our Rust types matches what WGSL expects.
    /// By referencing these constants, we activate their compile-time asserts.
    #[test]
    fn test_memory_layout_assertions() {
        // Just referencing these constants will include them in the build and 
        // trigger the static assertions they contain
        let _ = layout_asserts::WGSL_BASE_TYPE_ASSERTS;
        let _ = layout_asserts::PBR_CAMERA_UNIFORM_ASSERTS;
        let _ = layout_asserts::PBR_MODEL_UNIFORM_ASSERTS;
        let _ = layout_asserts::PBR_MATERIAL_UNIFORM_ASSERTS;
        
        // Additional runtime checks for key struct sizes
        ...
        // Verify critical field offsets at runtime too
        assert_eq!(std::mem::offset_of!(CameraUniform, view_proj), 0);
        assert_eq!(std::mem::offset_of!(CameraUniform, position), 64);
        
        assert_eq!(std::mem::offset_of!(ModelUniform, model), 0);
        assert_eq!(std::mem::offset_of!(ModelUniform, normal_transform), 64);
        
        assert_eq!(std::mem::offset_of!(MaterialUniform, base_color_factor), 0);
        assert_eq!(std::mem::offset_of!(MaterialUniform, metallic_factor), 16);
        assert_eq!(std::mem::offset_of!(MaterialUniform, roughness_factor), 20);
    }
