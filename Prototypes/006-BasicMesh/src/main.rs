...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let shader_id = window.register_shader(shader_descriptor);    // Create cube geometry using the GeometryFactory
    // Use StandardShaderVertex (re-exported in standard_geometry::prelude)
    let cube_geometry = GeometryFactory::cube(1.0, Vec3::ONE); // Create a 1x1x1 white cube

    // --- Camera Setup ---
    let initial_width = 1024.0;
    let initial_height = 768.0;
    let aspect_ratio = initial_width / initial_height;
    // Camera setup using glam Vec3
    let camera = Camera::new(
        Vec3::new(10.0, 10.0, 15.0), // eye position
    ...
    window.with_frame_callback(move |canvas: &mut Canvas| {
        ...
        for x in -GRID_SIZE / 2..GRID_SIZE / 2 {
            for z in -GRID_SIZE / 2..GRID_SIZE / 2 {
                let base_pos = Vec3::new(x as f32 * SPACING, 0.0, z as f32 * SPACING);
                ...
                let instance_pos = base_pos + Vec3::new(0.0, y_offset, 0.0);
                let model_matrix = Mat4::from_translation(instance_pos);
                ...
                // Create the instance data struct expected by StandardShader
                // Convert glam types to raw arrays
                let instance_data = StandardShaderInstances {
                    model_matrix: model_matrix.to_cols_array_2d(), // Convert Mat4 to [[f32; 4]; 4]
                    instance_color,
                };
